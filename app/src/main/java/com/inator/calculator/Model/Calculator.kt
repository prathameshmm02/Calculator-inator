...
class Calculator {
    ...
    private var addToOperand = false
    fun solve(expr: String?, isDegree: Boolean): BigDecimal {
        ...
        while (i < expr!!.length) {
            ...
            if (addToOperand) {
                ...
                if (current != ')' && !(basicOperators.contains(previous) || basicOperators.contains(
                        next
                    ) || previous == '|' || next == '|')
                ) {
                    operatorList.add('×')
        ...
        for (operator in operators) {
            ...
            while (operatorList.contains(operator)) {
                ...
                when (operator) {
                    '√' -> {
                        answer = BigDecimal.valueOf(sqrt(num1.toDouble()))
                    }
                    '!' -> {
                        answer = BigDecimal(factorial(num1.toBigInteger()))
                    }
                    else -> {
                        num2 = try {
                            operandList[index + 1]
                        ...
                        answer = when (operator) {
                            ...
                            '%' -> {
                                num1.multiply(num2).divide(BigDecimal.valueOf(100), MathContext.DECIMAL128)
                            }
                            else -> {
                                num1
                            }
